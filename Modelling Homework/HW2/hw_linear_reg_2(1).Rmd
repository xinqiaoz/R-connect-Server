...
```{r fig.height=6}
aa1 = ggplot(hi,aes(y=log(earn,10),x=h_over_m,color = as.factor(sex))) + geom_point() + geom_smooth(method = 'lm')
...
```{r}
ggplot(hi,aes(y=log(earn,10),x=ed,color = as.factor(sex))) + geom_point() + geom_smooth()
```
...
```{r fig.height=3.8}
mu = lm(log(earn,10)~h_over_m+sex+I(log(ed))+sex:h_over_m+ed:sex,data = hi)
summary(mu)
ggplot()+geom_histogram(aes(x=mu$residuals))
car::qqPlot(mu$residuals, envelope = 0.99,grid=F)
```
